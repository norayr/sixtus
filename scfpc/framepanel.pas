...
uses
  uLng, uConv, uShowMsg, uGlobs, GraphType, uPixmapManager, BaseUnix;
...
{  (Sender as TDrawGrid).Canvas.TextOut(Rect.Left, Rect.Top, IntToStr(ARow));
  ...
  begin
    ...
    begin
      ...
      Canvas.TextOut(Rect.Left, Rect.Top, FHeaderString[ACol]);
    end;
    Exit;
  end;
...
//    Canvas.FillRect(Rect);
    ...
    4:
      begin
        Canvas.TextOut(Rect.Left,Rect.Top,sModeStr);
      end;
    ...
    0:begin
      if iIconID>=0 then
        begin
          bmp:=PixMapManager.GetBitmap(iIconID);
...
begin
  btnUp.Left:=pnlHeader.Width-btnUp.Width-5;
  btnHome.Left:=btnUp.Left-btnHome.Width-4;
  btnRoot.Left:=btnHome.Left- btnRoot.Width-4;
end;

procedure TFrameFilePanel.btnHomeClick(Sender: TObject);
begin
  pnlFile.ActiveDir:=GetEnvironmentVariable('HOME')+'/';
  pnlFile.LoadPanel;
end;

procedure TFrameFilePanel.btnRootClick(Sender: TObject);
begin
  pnlFile.ActiveDir:='/';
  pnlFile.LoadPanel;
end;

procedure TFrameFilePanel.btnUpClick(Sender: TObject);
begin
  pnlFile.cdUpLevel;
end;
...
//  lblFree.AutoSize:=True;
  ...
  btnUp.Width:=20;
  btnUp.Left:=pnlHeader.Width-44;
  btnUp.Caption:='..';
  btnUp.Anchors:=[akRight];

  btnHome:=TSpeedButton.Create(pnlHeader);
  btnHome.Parent:=pnlHeader;
  btnHome.Width:=20;
  btnHome.Left:=pnlHeader.Width-66;
  btnHome.Caption:='~';
  btnHome.Anchors:=[akRight];
