...
uses
  SysUtils, Dialogs, uFindMmap, uLng, BaseUnix;
{ TFindThread }
...
begin
  writeln(sNewDir);
  if fpChdir(PChar(sNewDir))<>0 then Exit;
  ...
  repeat
    ...
    begin
     if FFilter.CheckFileMask(sr.Name) and FindMmap(sNewDir+'/'+sr.Name,FFindData,FCaseSens) then
      begin
        fFoundFile:=sNewDir+'/'+sr.Name;
        Synchronize(@AddFile);
      end;
    end
    else
    ...
    FCurrentFile:=sNewDir+'/'+sr.Name;
    Synchronize(@UpDateProgress);
    if (sr.Attr and faDirectory)>0 then
      WalkAdr(sNewDir+'/'+sr.Name);
