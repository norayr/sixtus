...
{  if ParamCount<>2 then
  begin
    ...
    GetMem(pMem,imem+2);
    try
      PrintCaps(VFSCaps(pmem,'.rpm'));
//      if VFSOpen(pmem,PChar('scbin1.tar.gz'))<>cVFS_OK then Exit;
      if VFSOpen(pmem,PChar('unrar-2.71-1mdk.i686.rpm'))<>cVFS_OK then Exit;
      ...
      VFSCopyOut(pmem,'info.txt','c:\temp\info.txt');
      VFSCopyOut(pmem,'data.cpio.gz','c:\temp\data.cpio.gz');
